// WizardNav — the floating Back / Next dock, ported from the Universe
// modelling tool (client/src/components/WizardNav.jsx). Fixed to the lower
// right of the viewport, it fades out after a short idle gap and returns on
// any activity; CSS keeps it reachable via :hover / :focus-within while
// faded. Anything passed as children (a Save button) sits between the two.
  ...
  return (
    ...
    >
      {hasPrev && (
        ...
        >
          <span aria-hidden="true">← </span>{backText || (backLabel ? `Back: ${backLabel}` : 'Back')}
        </button>
      )}
      {children}
      {hasNext && (
        ...
        >
          {nextText || (nextLabel ? `Next: ${nextLabel}` : 'Next')}<span aria-hidden="true"> →</span>
        </button>
      )}
    </nav>
  );
